...
from dealreg.loader import load, merge, resolve_headers, _read_rows  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--input", "-i", required=True, type=Path, nargs="+",
                    help="deal registrations: .csv, .xlsx, or .json/.jsonl query "
                         "results. Several may be given (e.g. one file per quarter "
                         "when a query is fetched in chunks); they are merged, and "
                         "rows repeated across them are de-duplicated by "
                         "registration id.")
    ...
    if args.describe:
        first = args.input[0]
        headers, rows = _read_rows(first)
        resolved, notes = resolve_headers(headers, mapping)
        if len(args.input) > 1:
            print(f"\n(describing {first.name}; {len(args.input) - 1} further input(s) "
                  f"will be merged with it)")
        print(f"\n{first.name}: {len(rows)} data rows, {len(headers)} columns\n")
        print("Detected column mapping")
        print("-" * 72)
        for fld in ("created_date", "rep", "isr", "pod", "stage", "deal_type",
                    "count", "deal_reg_id", "country", "amount", "partner",
                    "account", "close_date"):
    ...
    if q.get("pre_aggregated"):
        print(f"Input is pre-aggregated: {q['rows_in_window']} grouped rows in window "
              f"standing for {q['regs_in_window']} deal registrations "
              f"({q['regs_outside_window']} outside)")
    else:
